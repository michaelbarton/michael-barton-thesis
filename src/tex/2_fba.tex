...
The use of each of the single-gene encoded reactions was assessed in all three of the optimal and suboptimal nutrient limiting solutions. Each reaction was categorised based on whether the reaction was used in the solution and whether the reaction flux could be decreased whilst still maintaining the biomass growth rate. The numbers of reaction in each category are shown in Table~\vref{table:reaction_use}. The four reaction categories were:
...
\paragraph{Reaction rate at maximum.} Reactions at the allowable limit ($\pm$1000 mmol$^{-1}$ gDW$^{-1}$ hr$^{-1}$) for any reaction in the FBA solution. The numbers of these reactions was limited and was no more than four in any simulation.
...
Reaction flux was examined as a measure for reaction cost. Mutational effects in genes encoding high flux reactions may be expected to have a greater biological cost. Of the all the single gene-associated reactions, only a small subset exhibited non-trivial flux difference between glucose, ammonium and sulphate limitation. Furthermore of this subset ammonium and sulphate limitation displayed identical flux distributions, when estimated using the optimal FBA solution, indicating that the flux phenotype for single gene-associated reactions is the same for ammonium and sulphate limitation. The significant difference in flux distribution between ammonium and sulphate limitation must therefore be encoded my multimeric enzymes, or enzymes with a similar function. The glucose limited flux distribution for single gene-associated reactions was relatively lower than that of ammonium and sulphate limitation indicating that single encoded reaction, on average, carry less flux.

When estimating reaction flux using suboptimal simulation the glucose limited flux distribution remained relatively unchanged, while the sulphate and ammonium limitation flux decreased. This indicates that relaxing solution constraint has little effect on single gene-associated reactions in glucose limitation, but does reduce overall reaction flux in sulphate and ammonium limitation.

Comparing flux distribution between single gene associated reactions across nutrient limitation showed little correlation, with the exception of ammonium and sulphate limitation in optimal conditions which are identical. This should be taken into consideration given that only the significantly different reactions between conditions were compared, and overall in may be expected that the flux distributions for each environment are highly correlated.

\subsubsection{Gene cost through constraints on reaction flux}%{{{3

The constraints on decreases in reaction flux represent the edges of the FBA solution space when optimising the \emph{S. cerevisiae} model. The reactions at maximum flux represent the boundaries on model optimisation where any reaction cannot exceed $\pm$1000 mmol$^{-1}$ gDW$^{-1}$ hr$^{-1}$. The biological feasibility of whether a reaction may reach this level of flux is also questionable, given that the average reaction flux is much lower.  The number of reactions at maximum is low should be ignored when examining gene cost given that maximum reaction flux is not a biologically feasible phenotype.

The constrained reactions represent those whose flux is constrained in the solution space. Forced reduction of the flux through any of these reactions prevents the production of an FBA solution. As the flux cannot be reduced, these reactions all represent inviable knockout mutants where removal produces a lethal phenotype. These reactions may be expected to be functionally constrained \emph{in vivo} as the \emph{in silico} simulation suggests these reactions are the most sensitive reactions on growth. Comparing suboptimal FBA simulation there was still a significant number of constrained reactions, indicating constrained reactions are present even in a relaxed model optimisation.

The variable reactions are those where the flux can be reduced and still maintain a viable model solution. The variability of these reactions may be because alternate pathway for the production of a required nutrient exist, or the flux of another reaction may be increased. Changes in variable reaction fluxes represent non-unique flux distributions where multiple different combinations of flux distributions can produce the same model objective. \emph{In vivo} these variable reactions may represent metabolic flexibility or buffering in the flux phenotype. Using a suboptimal FBA solution identifies more flexible reactions, which may be expected as the solution space is less constrained.

The zero flux reactions are unused in the solution space and represent the majority of single gene-associated reactions. These reactions could be removed from the model without any effect on the FBA solution. These reactions could represent environment or nutrient specific reactions, required only when a specific nutrient is present. In may be expected that the \emph{in vivo} fitness effect of these reactions is specific to only the environments where the reaction is used.

Comparing the overlap of reaction type between each of the three nutrient limited environments indicated that the majority of reaction categories were consistent across environments. This suggests, in the conditions and single gene-associated reactions examined, that reaction constraints do not differ between environments. This may attributed to the core metabolic requirements of the biomass equation where specific nutrient synthesis is required independent of environment, placing consistent constraints on the same reactions.

\subsubsection{Gene cost through phenotypic effects on nutrient uptake}%{{{3

Given that only a small fraction of all single gene-associated reactions could be varied in the FBA solution, the number of reactions for which a sensitivity could be estimated was relatively small in comparison with overall metabolism. Of this small set of reactions few had a sensitivity close to 1, where the median reactions sensitivity was $\sim10^{-8}$. This indicates of the variable reactions of which sensitivity was estimated, the majority of reactions produced only minor effects on nutrient uptake when reaction flux was reduced.

The most sensitive nutrient uptake to changes in reaction flux was glucose. This may be expected that glucose uptake is responsible for the production of energy used in the other cellular biosynthetic processes. The ammonium and sulphate uptakes might be expected to only be sensitive to changes in reactions that produce metabolites requiring nitrogen or sulphur.

The reason that most reactions have little effect on nutrient uptake flux could be attributed to biological robustness, where metabolism is able to effectively buffer itself against changes reaction activity. However this is not the case for the large number of high constrained reactions, described in the previous section, where any changes in reaction flux are not tolerated by the metabolic network and an optimal FBA solution cannot be found. The estimation of reaction sensitivity is likely to highly dependent on the solution space when optimising a stoichiometric model. The sensitivity of reaction is highly dependent on how the model was optimised, this is readily apparent in he comparison of optimal and suboptimal reaction sensitivity for the same reactions, where there was little correlation between reactions. A further consideration is the number of small flux reactions in a FBA solution space. These reactions may carry a tiny flux in the solution which may not be representative of \emph{in vivo} conditions, but is instead an artifact of the model optimisation process.

\clearpage

\section{Conclusion}%{{{1

This chapter has outlined how genome scale models and flux balance analysis can be used to estimate both the cost of an amino acid and the fitness effect of a gene. Amino acid cost estimates are correlated with previous measures of amino acid cost produced from manual curation of metabolic maps. In comparison, gene cost has widely been interpreted in terms of dispensability: if reaction removal has a lethal phenotype. Here gene cost has been interpreted using reaction flux, constraints of reaction flux, and the effect of small changes in reaction flux on nutrient uptake.

Amino acid cost in gene expression and evolution considered in the following chapter. Gene cost on evolutionary rate and haploinsufficiency is considered in the chapter after that.
