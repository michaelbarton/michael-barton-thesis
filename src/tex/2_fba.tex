...
Nutrient limitation was simulated by fixing the upper and lower bounds of the biomass production reaction to 0.3$^-1$, and changing the objective function to minimise the entry of given nutrient into the cell. All other nutrient exchange fluxes were set to have bounds of -10000 mmol$^{-1}$ gDW$^{-1}$ hr$^{-1}$, the aim of this is to set all other nutrients to be non limiting on simulated growth. Model optimisation therefore finds the minimum required amount of a given nutrient intake flux to maintain cellular growth rate at 0.3 hr$^{-1}$.

For the \emph{S. cerevisiae} iND750 \cite{duarte2004a} genome scale model the default reactions allowing entry of nutrients into the cell are glucose, ammonium, sulphate, phosphate, oxygen and water. Each exchange flux represents the only source for an essential nutrient, and therefore when estimating the glucose cost of an amino acid there were no other high energy sugars available, or when estimating the ammonium cost there were no other nitrogen sources entering the cell. The FBA simulations do however allow a variety of molecules to exit the cell. In the simulations outlined here only glucose, ammonium and sulphate limiting conditions were simulated.

Fixing growth rate all \emph{in silico} also simulates experimental conditions of using continuous culture of microorganisms in the chemostat (see Castrillo \emph{et al.} \cite{castrillo2007}). The minimisation of a given nutrient as the \emph{in silico} objective mimics this condition where the nutrient is the limiting factor for growth and all other nutrient uptake fluxes are unbounded.

\subsection{Flux balance analysis estimation of amino acid cost}

Estimation of amino acid cost was performed by making a percentage change (0.0002\%) to the requirement of amino acid at position $S_{ij}$ in the stoichiometric matrix $S$, where $i$ is the amino acid and $j$ is the biomass reaction. When the model is optimised each change in $S_{ij}$ results in a corresponding change in the limiting nutrient uptake. Larger effects in nutrient uptake indicate a greater nutrient cost for the amino acid.  The slope between the change in amino acid requirements and the corresponding change in nutrient uptake flux was taken as the relative cost of the amino acid in that nutrient limiting condition. Division of the absolute cost by the original requirement results in an absolute estimate of amino acid cost. For each nutrient this results in a relative and absolute cost of each amino acid. Each cost was divided by the growth rate at which it was estimated (0.3 hr$^{-1}$) to give an estimate of cost independent of growth. The nomenclature used for these cost definitions is $R_{nutrient}$ and $A_{nutrient}$ respectively.
\nomenclature{$A_{nutrient}$}{Amino acid absolute nutrient cost}
\nomenclature{$R_{nutrient}$}{Amino acid relative nutrient cost}

%TODO: LaTeX plus/minus sign for percentage
