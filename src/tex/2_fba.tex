...
This chapter illustrates a novel approach for estimating the cost and importance of amino acids and genes \emph{Saccharomyces cerevisiae}. The novel methodology described applies existing systems biology control frameworks to the reconstruction of the yeast metabolism. The results determine the energetic and material costs of amino acid synthesis and the constraint and use of genes in different growth conditions.

The background section describes metabolic control analysis and how this is applied to how the cell controls metabolism. This is followed by a description of genome scale model construction and how flux balance analysis is used to simulate \emph{in silico} metabolism. The results section first outlines a novel method to estimate the cost of amino acid synthesis in glucose, ammonium, and sulphur limitation, corresponding to the energetic, nitrogen and sulphur content of the amino acid. The second half of the results section describes a method to determine the use of gene-encoded reactions in glucose and ammonium limited conditions the importance of a reaction is relevant to predicting the selection pressures on the encoding gene.
...
Early enzymology assumed the existence of rate limiting steps in biological pathways. The intuition was that the overall rate of a pathway is constrained by the rate of the slowest step. As the slowest reaction rate increases the whole pathway rate increases in proportion until another step becomes limiting. Niederberger \emph{et. al} \cite{niederberger1992} tested this assumption and found that individual up or down regulation of specific enzyme quantities had only marginal effect on overall tryptophan biosynthesis in \emph{Saccharomyces cerevisiae}. The rate of the pathway could instead be accelerated by increasing the quantity of five related enzymes in tandem. This research demonstrated that control in a biological system is distributed over the system as a whole rather than concentrated at individual reactions.

The theory of metabolic control analysis (MCA) \cite{fell1992,fell1997} states that there is no rate limiting step in a pathway, but instead each reaction shares a measure of overall control of metabolism. The control the rate of example reaction $x$ has on reaction $y$ is determined through an small change in the rate of $x$ and measuring the degree of response in the rate of $y$. In metabolic control analysis this is described as the flux control of reaction $x$ on reaction $y$. The coefficient of this control uses the symbol $C_{x}^{y}$ and is:

\begin{equation}\label{fcc}
C_{x}^{y} = \frac{ \delta \ln x / \ln x}{\delta \ln y / \ln y} \, .
\end{equation}

The numerator $\delta \ln x / \ln x$ represents the unitless effect of a small change in the reaction flux through $x$. The denominator $\delta \ln y / \ln y$ represents the corresponding unitless response in $y$ resulting from the change in $x$. The value of $C_{x}^{y}$ represents the degree of response in reaction $y$ to changes in the rate of $x$. Larger values of $C_{x}^{y}$ indicate that reaction $x$ has a high degree of control on reaction $y$. Smaller values indicate that reaction $y$ experiences only a small change in flux in response to changes in $x$.
\nomenclature{MCA}{Metabolic Control Analysis}
\nomenclature{$C_{x}^{y}$}{Flux control coefficient of reaction $x$ on reaction $y$}

Determining a numeric value for the relationships between reactions in a metabolic network allows properties of biological systems to be explored quantitatively. The control of metabolism can be into reactions whose rates are increased by changes in substrate concentration or the reactions whose activity is instead controlled through changes in enzyme activity and therefore maintained via gene expression \cite{rossell2006,daran-lapujade2007}.

An obvious example of the application MCA is engineering an increase in the production of biomolecules with commercial value \cite{niederberger1992}. MCA can also be used to identify drugs targets where reducing the flux combinations of high flux control reactions has the greatest effect on the metabolic network \cite{lehar2008,hopkins2008}. This can be further expanded in developing anitibiotic drugs for human pathogens such as trypanosome parasites. Ideal drug targets are those reactions with a high degree of control in the pathogen but where the orthologous reaction in the human host has a lower degree of control. The difference in control coefficients for the same reaction in both species translate into lethal drug effect in the parasite with minimal impact in the host \cite{hornberg2007}.

%TODO: read Hopkins, Lehar papers

\subsection{Genome scale models}

Derivation of a flux control coefficient \emph{in silico} requires a kinetic model with enzyme kinetic parameters for each reaction, in the case of simple Michaelis-Menten kinetics only substrate affinity (K$_{m}$) and maximum enzyme reaction rate (V$_{max}$) is necessary. More complex enzymatic reactions requires larger numbers of parameter sets which require further experimental effort to derive. Even with all experimentally derived kinetic parameters, models must be tested to determine if it matches expected \emph{in vivo} behaviour \cite{teusink2000}. The experimental effort required to derive the parameters to model each enzymatic reaction means the kinetic models of metabolic systems are relatively small compared with the anticipated size of overall cellular metabolism \cite{steuer2007}.
...
In comparison with kinetic models, a stoichiometric model is represented by an $m \times n$ sized matrix $S$ of representing reaction connectivity. The matrix $S$ represents the reactions in the metabolism and the metabolites that participate in the reactions. The size of of $m$ is the total number of metabolites in the model, and $n$ is the total number of reactions. The position $S_{ij}$ is the participation of metabolic $i$ in reaction $j$. Positive values of $S_{ij}$ indicate the metabolite is produced by the reaction, negative values mean the metabolite is consumed by the reaction. A zero value indicates the metabolite does not participate in the reaction. Any additional specific enzyme kinetic data is not necessary for stoichiometric model construction.

As the majority of a stoichiometric model is constructed from the connectivity between reactions and substrates, these types of models are generally much larger than kinetic models. Representations a significant proportion of an organism's metabolism have be created, described as genome scale models. The construction of genome scale metabolic models is reviewed by Feist \emph{et al.} \cite{feist2009}, and the process is outlined in brief here.

The construction of a genome scale model requires the production of the matrix $S$ detailing the participation of reactions and substrates in the organism's metabolism. Non species specific enzyme databases such as BRENDA \cite{chang2009} and KEGG \cite{okuda2008} provide information on the known interactions enzymes and pathways. The genome of the organism, or if available species specific databases such as the \emph{Saccharomyces} Genome Database (SGD) \cite{cherry1997}, allow the addition or removal of reactions to model, specific to that of the organism being reconstructed. The initial stages construction can be automated computationally, however expert knowledge of the organism can aid questions about the existence of specific metabolic reactions. This is particularly the case for organisms living in extreme growth environments \cite{sun2009}.
\nomenclature{SGD}{\emph{Saccharomyces} Genome Database (see \cite{cherry1997})}

The first \emph{Escherichia coli} model was developed almost 20 years ago \cite{feist2008}. A simplified model of acetate production was described by Majewski \emph{et al.} in 1990, and the recent 2007 genome scale model accounts for 1260 open reading frames \cite{feist2007}. The range species for which a genome scale model exists continues to expand and includes attempts to create a human cell reconstruction \cite{duarte2007}. The field of genome scale model construction unfortunately suffers from the lack of a single standard for construction and format of models. This can make model interchange and comparison difficult. There are however attempts to create a unified \emph{S. cerevisiae} model \cite{herrgard2008} using existing standards such the systems biology markup language (SBML) as a format for creating models \cite{sbml}, minimum information requested in annotation of biochemical models (MIRIAM) as a standard for model creation and annotation \cite{lenovere2005}, and standardised encoding and representation of chemicals using the international chemical identifier (InChI) codes \cite{coles2005}.
...
If the intended purpose for construction of a genome scale model is the simulation of \emph{in vivo} phenotypes, an additional reaction describing cellular growth is required. In many cases this reaction consumes the constituent lipids, proteins, nucleic acids and other cofactors required and results in a per hour (hr$^{-1}$) production new biomass. The required quantities of each molecule are estimated from \emph{in vivo} millimoles per gram of dry weight cellular biomass (mmol $^{-1}$ gDW $^{-1}$). Prior to completion of a metabolic model \emph{in silico} simulations must be validated against the observed \emph{in vivo} phenotypes, where \emph{in silico} behaviour should reflect that of \emph{in vivo} behaviour as closely as possible \cite{famili2003}. Differences between \emph{in silico} and \emph{in vivo} can be used to identify gaps and then update the constructed model. The use and simulation of stoichiometric models is described in the next section.
...
Stoichiometric models are simulated using flux balance analysis (FBA). As described above a stoichiometric model is defined by a matrix $S$ describing the substrates and products used in each reaction. FBA uses linear programming to solve the equation $S v = 0$ where $v$ represents a combination of reaction rates, also known as fluxes, for each reaction in $S$. There are multiple solutions to this equation and so linear optimisation is used to determine the combination of fluxes for $v$ that maximises or minimises a particular value in the solution, this optimised reaction is described as the objective function. A common objective function used for genome scale models is maximising biomass production. Additional constraints can be placed on the flux balance solution, such as upper an lower boundaries for individual reactions. Constraining a reaction to be $\geq 0$ restricts the possible solution space so the reaction may only proceed in the forward direction. Reaction constraints are often used in this way to limit the flux of nutrients entering the cell and thereby simulate different environmental conditions.
\nomenclature{FBA}{Flux Balance Analysis}

The aim of flux balance analysis using a genome model is accurate simulation of organism phenotypes. Minimisation of metabolic adjustment (MOMA) \cite{segre2002,burgard2003} is a technique that optimises the objective function while minimising the sum change in flux adjustment between two flux solutions. The aim of this method to compare different predictions of metabolic phenotypes such a wild-type model and mutant reaction knockout model. MOMA attempts to simulate the effect of changes in metabolism where minimal flux changes more accurately reflect the \emph{in vivo} behaviour of how metabolic adjustments are made. A related approach minimises the number of on-off changes in reaction use (Regulatory on/off minimisation ROOM), whereby the number of reactions that switch between active or inactive between solutions is minimised \cite{shlomi2005}. This approach again mimics \emph{in vivo} behaviour where the use of already active reactions in different flux phenotypes is more realistic simulation than large scale metabolic readjustments in reaction activity.
...
Estimating control coefficients using stoichiometric models is difficult as the flux the through the reaction is determined by linear optimisation and constraints, rather than enzyme kinetics. A common approach to estimating the importance of a reaction in a genome scale model is to remove a reaction from the model, and compare the resulting solution with the original wild-type simulation. A reaction deletion may be considered to confer a fitness effect if there is a growth rate reduction in the mutant simulation compared with the wild-type simulation \cite{pal2006}.  The deletion of a reaction may even prevent model optimisation indicating the reaction is essential for the objective function \cite{becker2008}.  Even if a model solution can still be found, the reaction may still be considered essential to growth if growth effect of reaction removal is large enough \cite{papp2004,wang2009}.

Flux balance analysis of all possible gene deletions for \emph{S.cerevisiae} were calculated and compared with \emph{in vivo} observations and $\sim$85\% of knockout phenotypes were correctly predicted depending on the simulated media \cite{famili2003,forster2003}. Reactions that buffer the effects of each other can be determined by comparing all possible double deletions and finding the pairs with no phenotypic effect when deleted individually, but predict a sick or lethal phenotype when deleted simultaneously \cite{harrison2007}. This can be further extended with multiple reaction knockouts \cite{deutscher2008}. Reactions which may be active in a specific range of conditions can be assessed through deletion of reactions in a wide range of simulated conditions, where a deletion may result in a fitness effect in only a small range of biologically feasible conditions \cite{papp2004}.

The use of genome scale metabolic models is not limited to estimating the control of reactions on growth, but may also be used to estimate the importance of individual metabolites. Varma \emph{et al.} \cite{varma1993} used flux balance analysis to estimate the importance of oxygen to energy metabolism in \emph{E. coli} through simulating a decrease in oxygen availability and identifying the corresponding change in the use of redox carriers. As oxygen availability decreased, the use of ethanol as the electron acceptor increased. The author described this effect in terms of the price of the each molecule, where with decreasing oxygen levels the price of oxygen increase, while the price of ethanol decreased. The article by Varma \emph{et al.} highlights how systems biology models can be used to estimate the importance of a set of metabolites to cell growth. Few subsequent studies have looked at the price of metabolite in a systems biology framework, though the cost of amino acid to protein synthesis has been examined.
